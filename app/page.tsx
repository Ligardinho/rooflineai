...
export default function HomePage() {
  ...
  const [connectionResults, setConnectionResults] = useState<ConnectionResults>({ imap: null, smtp: null });
  ...
  const groupEmailsByConversation = (emails: Email[]) => {
    ...
    Object.values(conversations).forEach(conversation => {
      conversation.sort((a, b) => ensureDate(a.date).getTime() - ensureDate(b.date).getTime());
    });
  ...
  // --- NEW: Helper for email status ---
  const getEmailStatus = (email: Email) => {
    if (email.replied) return { text: 'Replied', bg: 'bg-green-500' };
    if (email.from.includes('@gmail.com')) return { text: 'Sent', bg: 'bg-blue-500' };
    return { text: 'Pending', bg: 'bg-orange-500' };
  };
  ...
  // SSE implementation for real-time email updates
  const startEmailStream = () => {
    const eventSource = new EventSource('/api/emails/stream');

    eventSource.onmessage = event => {
      try {
        const newEmail: Email = JSON.parse(event.data);
        setEmails(prev => [...prev, newEmail]);
        setNotification(`New email from ${newEmail.from}`);
        setTimeout(() => setNotification(''), 3000);
      } catch (err) {
        console.error('Error parsing streamed email:', err);
      }
    };

    eventSource.onerror = (event: Event) => {
      console.error('Email stream error:', event.type, event);
      // Optional: try reconnecting after a delay
      setTimeout(() => {
        eventSource.close();
        startEmailStream();
      }, 5000);
  ...
  const checkForNewEmails = async () => {
    ...
    try {
      ...
      if (data.newEmails && data.newEmails.length > 0) {
        setEmails(prev => [...prev, ...data.newEmails]);
          ...
          <div className="bg-white p-5 rounded-xl shadow-sm lg:col-span-1">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                ...
                <span className="font-bold text-green-600">{emails.filter(e => e.replied).length}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-amber-50 rounded-lg">
                <span className="text-gray-600 text-sm">Conversations</span>
                <span className="font-bold text-amber-600">{Object.keys(emailConversations).length}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-purple-50 rounded-lg">
                <span className="text-gray-600 text-sm">Pending Replies</span>
                <span className="font-bold text-purple-600">{emails.filter(e => !e.replied && !e.from.includes('@gmail.com')).length}</span>
              </div>
            </div>
            ...
            <div className="mt-6 space-y-3">
              ...
              >
                {isLoading ? <FiRefreshCw className="animate-spin mr-2" /> : <FiCheck className="mr-2" />}
                Test Connections
              </button>
              ...
              >
                {isLoading ? <FiRefreshCw className="animate-spin mr-2" /> : <FiRefreshCw className="mr-2" />}
                Check Emails
              </button>
            </div>
                ...
                {Object.entries(emailConversations).map(([threadId, conversation]) => (
                  <div key={threadId} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    <h3 className="font-semibold text-gray-800 mb-3 flex items-center text-sm">
                      ...
                      <span className="ml-2 text-xs font-normal text-gray-500">({conversation.length} messages)</span>
                    </h3>
                    <div className="space-y-2">
                      {conversation.map(email => {
                        ...
                        return (
                          <div
                            key={email.id}
                            className={`p-3 rounded-lg border-l-4 ${email.from.includes('@gmail.com') ? 'bg-blue-50 border-blue-400' : 'bg-gray-50 border-gray-400'}`}
                          >
                            ...
                            <span className={`text-xs font-medium px-2 py-1 rounded-full text-white ${status.bg}`}>
                              {status.text}
                            </span>
                          </div>
                        );
                  ...
                  return (
                    <div key={email.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <span className="text-xs text-gray-500">{ensureDate(email.date).toLocaleDateString()}</span>
                      </div>
                      <p className="text-gray-600 mb-1 text-xs">From: {email.from}</p>
                      <p className="text-gray-700 mb-2 text-xs line-clamp-2">
